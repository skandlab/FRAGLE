...
# Resolve paths using FRAGLE_HOME
fragle_home = os.environ.get("FRAGLE_HOME", ".")
meta_info_path = os.path.join(fragle_home, "meta_info", "meta_info.pkl")
models_dir = os.path.join(fragle_home, "Models")
...
class NN(nn.Module):
    def __init__(self):
        ...
        mlp_layers = []
        for _layer in range(self.number_mlp_layers):
...
LT_model = load_model(os.path.join(models_dir, "LT_model.pt"))
HT_model = load_model(os.path.join(models_dir, "HT_model.pt"))
select_model = load(os.path.join(models_dir, "select_model.pkl"))

# data loading and feature generation
data_dict = {}
data_path = os.path.join(input_folder, "data.pkl")
...
filter_window = 32
signal_pad = np.pad(
    data_X, ((0, 0), (filter_window // 2, filter_window // 2)), mode="edge"
)
strided_view = np.lib.stride_tricks.sliding_window_view(
    signal_pad, filter_window, axis=1
)
...
def predict_tf(model, model_type):
    ...
    with torch.no_grad():
        for i in range(data_X.shape[0]):
            ...
            if model_type == "LT":
                score = score.item()
                csv_path = os.path.join(output_folder, "LT.csv")
            elif model_type == "HT":
                score = score.item() * TF_std + TF_median
                csv_path = os.path.join(output_folder, "HT.csv")
